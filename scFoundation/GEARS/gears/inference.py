...
def evaluate(loader, model, uncertainty, device):
    ...
    for itr, batch in enumerate(loader):
        ...
        with torch.no_grad():
            if uncertainty:
                p, unc = model(batch)
                logvar.extend(unc.detach().cpu())
            else:
                p = model(batch)
            t = batch.y
            pred.extend(p.detach().cpu())
            truth.extend(t.detach().cpu())
            
            # Differentially expressed genes
            de_indices = torch.tensor(batch.de_idx)
            pred_de.extend(p[:, de_indices].detach().cpu())
            truth_de.extend(t[:, de_indices].detach().cpu())
            # for itr, de_idx in enumerate(batch.de_idx):
            #     pred_de.append(p[itr, de_idx].detach().cpu())
            #     truth_de.append(t[itr, de_idx].detach().cpu())

    # all genes
    results['pert_cat'] = np.array(pert_cat)
    pred = torch.stack(pred)
    truth = torch.stack(truth)
    results['pred']= pred.numpy()
    results['truth']= truth.numpy()

    pred_de = torch.concat(pred_de)
    truth_de = torch.concat(truth_de)
    results['pred_de']= pred_de.numpy()
    results['truth_de']= truth_de.numpy()
    
    if uncertainty:
        results['logvar'] = torch.stack(logvar).numpy()
